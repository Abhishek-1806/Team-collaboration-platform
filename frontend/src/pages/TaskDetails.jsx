...
const TaskDetails = () => {
    ...
    if (role === "Admin") {
      const fetchUsers = async () => {
        ...
        try {
          ...
          setUsers(response.data.find((name) => name.id === task.assignedTo));
        } catch (error) {
  ...
  return (
    <div className="p-5 max-w-4xl mx-auto">
      <button
        onClick={() => navigate(-1)}
        className="flex items-center mb-5 text-blue-500 hover:text-blue-600"
      >
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <p className="text-gray-600 mb-2">
          <strong>Assigned to:</strong> <span className="capitalize">{username}</span>
        </p>
      ...
      {showFilePreview && task.fileUrl && (
        ...
        >
          {fileType === 'pdf' ? (
            <div className="w-[90%] h-[90vh] bg-white">
              <iframe
