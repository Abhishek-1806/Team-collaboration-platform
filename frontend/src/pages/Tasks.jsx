...
const Tasks = () => {
  ...
  const fetchUser = async () => {
    try {
      ...
      setUser(response.data.id); // Set the logged-in user's ID
    } catch (error) {
  ...
  // Filter tasks by assignedBy field and status
  const filteredTasks = tasks.filter((task) => {
    const statusFilter = getStatusFromRoute();
    if (statusFilter) {
      return task.assignedBy === user && task.status.toLowerCase() === statusFilter;
    }
    return task.assignedBy === user;
  });
